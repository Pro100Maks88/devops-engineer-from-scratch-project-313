...
### :keyboard: Доступные команды

| Команда | Описание |
| --- | --- |
| `make run` | Поднимает контейнеры (приложение + БД) в фоновом режиме. Автоперезагрузка включена. |
| `make test` | Запускает тесты внутри контейнера с правильной переменной окружения DATABASE_URL. |
| `make lint` | Проверяет код через ruff. |
| `make fix` | Автоматически исправляет стилистические ошибки через ruff. |
| `make clean` | Останавливает контейнеры, удаляет тома и очищает uv.lock. |
